...
* 分层
  ...
  */
  ...
  2. 因为数据链路层是负责相邻节点的数据传输, 因此必须要知道相邻节点的MAC地址, 通过广播ARP请求数据包, 局域网内的所有主机都会收到, 收到的主机解析数据包, 如果目的IP与本机的IP地址相符的话, 就将本机的MAC地址填入并将目的地址与源地址互换, 作为响应ARP数据包发送给目的主机.
...
# tcpdump 观察DNS通信过程
# -i ens33 来指定监听的网卡接口
# -nt 输出选项, -n表示显示IP而不是域名, -t是不显示时间戳
# -s 500 设计抓取数据包的长度(默认是68个字节)
# port domain 端口过滤
...
![image-20200908204231621](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20200908204231621.png)
